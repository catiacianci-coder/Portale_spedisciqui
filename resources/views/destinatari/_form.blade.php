...
<div class="sq-profilo-page sq-page-preventivi">
    ...
    <form method="POST" action="{{ $action }}" id="form-destinatario" autocomplete="off" class="sq-profilo-cards-grid" style="grid-template-columns: 1fr;">
        @csrf
        @if ($method !== 'POST')
            @method($method)
        @endif
        <input type="hidden" name="id_comune" id="id_comune_dest" value="{{ $oldIdComune }}">
        ...
        <div class="sq-profilo-card sq-profilo-card--stacked">
            ...
            <div class="sq-profilo-card-stack">
                ...
                <div class="sq-profilo-field">
                    <label for="dest_cognome" class="sq-profilo-label">Cognome <span class="sq-profilo-req">*</span></label>
                    <input type="text" name="cognome" id="dest_cognome" class="sq-profilo-input" required maxlength="255" value="{{ old('cognome', $m->cognome ?? '') }}">
                </div>
                <div class="sq-profilo-field">
                    <label for="dest_telefono" class="sq-profilo-label">Telefono <span class="sq-profilo-req">*</span></label>
                    <input type="tel" name="telefono" id="dest_telefono" class="sq-profilo-input" required maxlength="30" value="{{ old('telefono', $m->telefono ?? '') }}">
                </div>
                <div class="sq-profilo-field">
                    <label for="dest_email" class="sq-profilo-label">Email <span class="sq-profilo-req">*</span></label>
                    <input type="email" name="email" id="dest_email" class="sq-profilo-input" required maxlength="255" value="{{ old('email', $m->email ?? '') }}">
                </div>
            </div>
        </div>
        ...
        <div class="sq-profilo-card sq-profilo-card--stacked">
            ...
            <div class="sq-profilo-card-stack">
                <div class="sq-profilo-field sq-profilo-field--suggest sq-profilo-mb-15">
                    <label for="dest_citta" class="sq-profilo-label">Città <span class="sq-profilo-req">*</span></label>
                    <input type="text" name="citta" id="dest_citta" class="sq-profilo-input" required maxlength="255" placeholder="Inizia a scrivere il comune…" value="{{ old('citta', $m->citta ?? '') }}">
                    <div class="sq-profilo-suggest" id="suggest_dest_citta" hidden></div>
                </div>
                <div class="sq-profilo-cap-pv-row">
                    <div class="sq-profilo-field sq-profilo-field--suggest sq-profilo-mb-0">
                        <label for="dest_cap" class="sq-profilo-label">CAP <span class="sq-profilo-req">*</span></label>
                        <input type="text" name="cap" id="dest_cap" class="sq-profilo-input" required maxlength="5" inputmode="numeric" value="{{ old('cap', $m->cap ?? '') }}">
                        <div class="sq-profilo-suggest" id="suggest_dest_cap" hidden></div>
                    </div>
                    <div class="sq-profilo-field sq-profilo-mb-0">
                        <label for="dest_provincia" class="sq-profilo-label">Prov. <span class="sq-profilo-req">*</span></label>
                        <input type="text" name="provincia" id="dest_provincia" class="sq-profilo-input sq-profilo-input--ro" required maxlength="2" value="{{ old('provincia', $m->provincia ?? '') }}" readonly tabindex="-1">
                    </div>
                </div>
                <div class="sq-profilo-addr-row">
                    ...
                    <div class="sq-profilo-field sq-profilo-mb-0">
                        <label for="dest_civico" class="sq-profilo-label">Civico <span class="sq-profilo-req">*</span></label>
                        <input type="text" name="civico" id="dest_civico" class="sq-profilo-input" required maxlength="10" value="{{ old('civico', $m->civico ?? '') }}">
                    </div>
                </div>
        ...
        <div class="sq-profilo-global-actions" style="justify-content:flex-start;">
            ...
            <a href="{{ route('destinatari.index') }}" class="sq-btn-secondary sq-modal-btn" style="margin-left:10px;">Annulla</a>
        </div>
    </form>
</div>
...
(() => {
    ...
    function fillComune(it) {
        const hid = document.getElementById('id_comune_dest');
        if (hid) hid.value = String(it.id);
        const cap = document.getElementById('dest_cap');
        const cit = document.getElementById('dest_citta');
        const pv = document.getElementById('dest_provincia');
    ...
    function wireSuggestCap() {
        const input = document.getElementById('dest_cap');
        const list = document.getElementById('suggest_dest_cap');
    ...
    function wireSuggestCitta() {
        const input = document.getElementById('dest_citta');
        const list = document.getElementById('suggest_dest_citta');
        if (!input || !list) return;
        let controller = null;
        const run = debounce(async () => {
    ...
    document.getElementById('form-destinatario')?.addEventListener('submit', (e) => {
        const idc = (document.getElementById('id_comune_dest')?.value || '').trim();
        if (!idc) {
            e.preventDefault();
